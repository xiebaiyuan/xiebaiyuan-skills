...
>
> 💾 **现成处理脚本**：`scripts/ai_daily_process.py`（2026-08-18 实战版，含 AIHOT 同源去重、HN 合并、实体匹配越界防护、重点精选、分类分组输出）。用法：先跑上面两条 curl 落盘 /tmp/aihot.json + /tmp/hn_ids.json，再 `cp ~/.hermes/skills/research/aihot/scripts/ai_daily_process.py /tmp/ && python3 /tmp/ai_daily_process.py`，产物在 /tmp/ai_brief.md，最后 cp 到 vault 目标路径即可。
...
- **同源去重阈值**：SequenceMatcher ratio ≥0.62 直接合并；≥0.30 且共享 ≥6 字符英文 token（如 sbenergy）也合并——AI HOT 常同时收录 X 帖 + 官方 blog 同一新闻。合并时保留更详细的那条 summary（长度差 >15% 取长者，否则拼接），多余 URL 进「同源链接」行。
...
```bash
ls ~/AI_DOC/调研分析/*"$entity"*调研.md 2>/dev/null
ls ~/AI_DOC/wiki/entities/*"$entity"*.md 2>/dev/null
```

命中则用 `[[wikilink]]` 追加在条目末尾：
- `[[调研分析/{文件名}|📄 调研]]` 或 `[[wiki/entities/{实体名}|📄 wiki]]`
...
- 调研分析目录下只允许顶层文件命中（排除 `每日要闻/`、`技术调研/`、`产品对比/` 子目录）；同文件 wiki/entities 与 调研分析 双份存在时可都保留；每条目最多 2 个链接。
- 匹配不到就跳过，不编造 wiki 链接。
...
7. **`/usr/bin/python3` 是 Hermes venv shim，其 requests/urllib3 损坏**（2026-08-20 实测：`bytes | str` TypeError 崩溃 import），导致 `push-hiboard.sh` 的 `python3 scripts/task_push.py` 失败。解法：用 `/opt/homebrew/bin/python3`（requests 2.33.0 正常）执行 `task_push.py --data /tmp/push_hiboard_task.json`；JSON 由脚本用 stdlib json 构造即可。
